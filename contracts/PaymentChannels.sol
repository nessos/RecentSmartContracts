...
contract PaymentChannels is RecentBlockchain {
		...
		uint256 amount) public
  {
    ...
    require(signer == channels[channelId].sender, "Signer should be the channel Owner");
  ...
  //maxTxThroughput is the requested max allowed Offcain transactions throughput
  ...
  function requestRelayerLicense(uint targetEpoch, string memory domain, string memory name, uint fee, uint maxUsers, uint256 maxCoins, uint maxTxThroughput, uint offchainTxDelay) public payable {
    ...
    if (currentRelayersNumber >= maximumRelayersNumber) {
      ...
      for (uint i=1; i<=currentRelayersNumber; i++) {
      ...
      relayers[targetEpoch][relayersCounter[targetEpoch]].maxUsers = maxUsers;
      relayers[targetEpoch][relayersCounter[targetEpoch]].maxCoins = maxCoins;
      relayers[targetEpoch][relayersCounter[targetEpoch]].maxTxThroughput = maxTxThroughput;
      relayers[targetEpoch][relayersCounter[targetEpoch]].offchainTxDelay = offchainTxDelay;
      relayers[targetEpoch][relayersCounter[targetEpoch]].remainingPenaltyFunds = requiredAmount;
      relayers[targetEpoch][relayersCounter[targetEpoch]].name = name;
      relayers[targetEpoch][relayersCounter[targetEpoch]]. domain = domain;
      relayers[targetEpoch][relayersCounter[targetEpoch]].owner = msg.sender;
      epochRelayerIndex[targetEpoch][msg.sender] = relayersCounter[targetEpoch];
    }
  ...
  function relayerWithdrawPenaltyFunds(uint targetEpoch) public
  {
    ...
    require(targetEpoch < currentEpoch -1 , "Current epoch should be lower than requested epoch");
  ...
  mapping (address => mapping (address => mapping ( address => mapping (bytes32 => uint256)))) public userToBeneficiaryFinalizedAmountForNonce;
  ...
  function depositToRelayer(address relayerId, uint lockUntilBlock) public payable
  {
    ...
    if (userDepositOnRelayer[msg.sender][relayerId].lockUntilBlock == 0) {
      ...
      require(userDepositOnRelayer[msg.sender][relayerId].lockUntilBlock >= lockUntilBlock, "The lockTimeInDays should be greater than any previous lock until Block");
    }
		...
		uint256 amount) public pure returns (address signer)
  {
    ...
    //Recover the Offcain transaction Signer using ECDA public key Recovery(User signed the Tx)
    ...
    uint txUntilBlock) public pure returns (address payable relayerid)
  {
    ...
    //Recover the Offcain transaction Signer using ECDA public key Recovery(Relayer signed the Tx)
		...
		uint256 amount) public
  {
    ...
    require(userToBeneficiaryFinalizedAmountForNonce[signer][beneficiary][relayerId][nonce] < amount, "Requested amount should be greater than the previous finalized for withdraw request or P2P content transaction");
    
    //Get the delta between requested and the previously used of the same nonce 
    uint256 amountToBeTransferred =  amount - userToBeneficiaryFinalizedAmountForNonce[signer][beneficiary][relayerId][nonce];
