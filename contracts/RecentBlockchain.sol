...
contract RecentBlockchain {
    ...
    uint256 public witnessRequiredBalancePercent = 3;
    ...
    uint256 public freeServiceDisputeThreshold = 5000;
    ...
    function calculateReward(
        ...
        uint256 divisor = (issuanceBlock / halvingEvery) + 1;

        //THe block reward based of halving period
        uint256 blockReward = maxReward / divisor;
        ...
        uint256 totalReward = multiplier * blockReward;
    ...
    function calculateReward(uint256 epoch) public view returns (uint256) {
        ...
        uint256 issuanceBlock = epoch.mul(epochBlocks) - 1;

        //The last normally claimed block before halving period last block
        uint256 lastClaimedIssuanceBlock = issuanceBlock - 1;
    ...
    function getEpochByBlock(uint256 requestedBlock)
        public
        view
    ...
    //Get next Epoch
    function getTargetEpoch() public view returns (uint256 epoch) {
        return getCurrentEpoch() + 1;
    }

    //The Block number tha Current epoch ends
    function getCurrentEpochEnd() public view returns (uint256 epoch) {
